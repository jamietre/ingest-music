...
- [ ] Trim leading/trailing whitespace from venue names and locations (data quality fix)
...
- [ ] **Add unknown artist handling** - When encountering a previously unknown band:
  - [ ] Decide whether to add artist to config automatically or prompt user
  - [ ] Prompt user for genre
  - [ ] Prompt for pattern matching regex
  - [ ] Prompt for preferred setlist sources
  - [ ] Save to config file
- [ ] **Configuration for venue name/location shorthands** - Allow mapping long venue names to shorthands
  - [ ] Add config section for venue aliases/shorthands
  - [ ] Match found venue info against configured shorthands
  - [ ] Example: "William Randolph Hearst Greek Theatre, University of California, Berkeley" → "Greek Theatre Berkeley"
  - [ ] Support fuzzy matching for venue names
...
- [ ] Add integration tests that use LLM to test prompt engineering (separate pnpm script, not run by default)
