...
2. MERGE EVERY NON-SONG TRACK:
   ...
   - MERGE ORDER MATTERS: [A, B] means merge A into B and keep B's name
   - EXAMPLES:
     * Track 1 "DJ Intro" → suggest [1, 2] = merge 1 into 2, keep track 2's name
     * Track 4 "Guitar Intro" → suggest [4, 5] = merge 4 into 5, keep track 5's name
     * Track 10 "Banter #1" → suggest [10, 11] = merge 10 into 11, keep track 11's name
     * Track 8 "Song" + Track 9 "Outro" → suggest [8, 9] if keeping track 9, or [9, 8] if keeping track 8
...
- Each merge: { "tracks": [track_to_merge_in, track_to_keep] }
- ORDER MATTERS: [A, B] merges A into B and keeps B's name/position in setlist
...
- Examples:
  * [{ "tracks": [23, 24] }] → merge track 23 into 24, match against setlist position 24
  * [{ "tracks": [4, 5] }] → merge track 4 into 5, match against setlist position 5
  * [{ "tracks": [9, 8] }] → merge track 9 into 8, match against setlist position 8 (if 9 is outro)
...
MERGE ORDER MATTERS - Format: [A, B] means merge A into B, keep B's name:
- "merge track 4 into 5" → ONE merge: [4, 5]
- "merge track 5 into 4" → ONE merge: [5, 4]
- "merge tracks 4 and 5" → ONE merge: [4, 5] (default order keeps 5)

MULTIPLE MERGES - Semicolons or commas separate multiple operations:
- "merge 6 into 5; merge 11 into 10" → TWO merges: [{"tracks": [6, 5]}, {"tracks": [11, 10]}]
- "merge 4 into 5, merge 8 into 7" → TWO merges: [{"tracks": [4, 5]}, {"tracks": [8, 7]}]
- "merge 6 into 5 and merge 11 into 10" → TWO merges: [{"tracks": [6, 5]}, {"tracks": [11, 10]}]

CRITICAL PARSING RULES:
1. Each "merge X into Y" is a SEPARATE merge operation: [X, Y]
2. Semicolons, commas, or "and" separate multiple merge operations
3. "merge 6 into 5; merge 11 into 10" means TWO operations, NOT one
4. Track numbers are AUDIO FILE numbers (1-indexed), not setlist positions

Examples with expected output:
- "merge 4 into 5" → {"merges": [{"tracks": [4, 5]}]}
- "merge 6 into 5; merge 11 into 10" → {"merges": [{"tracks": [6, 5]}, {"tracks": [11, 10]}]}
- "split track 3 at 12:30" → {"splits": [{"track": 3, "timestamp": "12:30"}]}

RESPONSE FORMAT - YOU MUST INCLUDE ALL FIELDS:
{
  "modifiedSetlist": null,  // null if no setlist changes, OR array of songs if modified
  "merges": [
    { "tracks": [6, 5] },   // First merge: 6 into 5, keep 5
    { "tracks": [11, 10] }  // Second merge: 11 into 10, keep 10
  ],
  "splits": [],  // Empty array if no splits, OR array of split operations
  "reasoning": "Merged audio track 6 into 5 (keeping track 5's name 'Cities'), merged audio track 11 into 10 (keeping track 10's name 'Rock and Roll')",
  "confidence": 0.95
}

CRITICAL REQUIREMENTS:
1. ALWAYS include "merges" field - empty array [] if no merges, OR array of merge objects if merging
2. ALWAYS include "splits" field - empty array [] if no splits
3. ALWAYS include "modifiedSetlist" field - null if no changes, OR array if modified
4. If user says "merge track 1 into track 2", YOU MUST include: "merges": [{"tracks": [1, 2]}]
5. DO NOT just explain merges in reasoning - you MUST include them in the "merges" array
6. If confidence < 0.5, explain why in reasoning

Example for "merge track 1 into track 2":
{
  "modifiedSetlist": null,
  "merges": [{ "tracks": [1, 2] }],
  "splits": [],
  "reasoning": "Merged audio track 1 into track 2 (keeping track 2's name)",
  "confidence": 1.0
}`;
  }
