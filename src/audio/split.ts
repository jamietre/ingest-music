...
export async function applyMergesToFiles(
  ...
  for (const merge of sortedMerges) {
    try {
      ...
      const trackNums = merge.tracks.map(t => t.track).join(", ");
      onProgress?.(
        `  Merging tracks ${trackNums}`
      );
      logger.info(`Merging tracks ${trackNums}`);

      // Create merged file path using the LAST track (the one we're keeping)
      // This matters for setlist matching - e.g., [4, 5] means merge 4 into 5, keep 5's name
      const lastFile = filesToMerge[filesToMerge.length - 1];
      const dir = path.dirname(lastFile);
      const ext = path.extname(lastFile);
      const base = path.basename(lastFile, ext);
      const mergedPath = path.join(dir, `${base}_merged${ext}`);

      // Merge the files
      await mergeAudioFiles(filesToMerge, mergedPath, onProgress);
      logger.info(`Successfully merged tracks ${trackNums} -> ${path.basename(mergedPath)}`);

      // Update file list: remove individual files, add merged file
      // Find min and max indices to handle any merge order (e.g., [6, 5] or [5, 6])
      const indices = merge.tracks.map(t => t.track - 1);
      const minIndex = Math.min(...indices);
      const maxIndex = Math.max(...indices);
      const numToRemove = maxIndex - minIndex + 1;
