...
export async function convertAllIfNeeded(
  ...
  logger.info(`Converting ${toConvert.length} audio file(s)`);
  ...
  for (const info of toConvert) {
    ...
    logger.info(`Converted: ${path.basename(info.filePath)} -> ${path.basename(newPath)}`);
  }
  logger.info(`Successfully converted ${toConvert.length} audio file(s)`);
  ...
  if (!isInTemp && targetDir !== workingDir.path) {
    const nonConverted = audioInfos.filter(info => !convertedPaths.has(info.filePath));
    if (nonConverted.length > 0) {
      logger.info(`Copying ${nonConverted.length} non-converted file(s) to temp directory`);
    }
    for (const info of nonConverted) {
      ...
      logger.info(`Copied: ${path.basename(info.filePath)}`);
    }
  }
