...
import { getAudioFormat } from "../audio/formats.js";
...
/**
 * Tag an MP3 file with metadata using node-id3.
 */
export async function tagMp3(
  ...
  try {
    // Build tags object
    // Note: keepTags is not supported for MP3 files (primarily a FLAC feature)
    const tags: any = {
      artist,
      album,
      performerInfo: albumArtist, // TPE2 frame (album artist)
      title,
      trackNumber: String(track.trackInSet),
      partOfSet: String(track.effectiveSet), // TPOS frame (disc number)
      genre,
      year,
    };

    // Write tags
    const success = NodeID3.write(tags, inputPath);
    if (!success) {
      throw new Error("node-id3 write returned false");
    }
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to tag ${path.basename(inputPath)}: ${msg}`);
...
export async function tagAllTracks(
  ...
  for (const track of tracks) {
    const format = getAudioFormat(track.audioFile.filePath);

    if (!format) {
      onProgress?.(`  Skipping: ${track.song.title} (unknown format)`);
      continue;
    }
    ...
    switch (format.extension) {
      case ".flac":
        ...
        break;
      case ".mp3":
        await tagMp3(track, showInfo, bandConfig, onProgress);
        break;
      default:
        onProgress?.(`  Skipping: ${track.song.title} (${format.name} tagging not yet supported)`);
    }
  }
}
...
export function buildTemplateVars(
  ...
  // Get file extension (without the dot) for use in templates
  const ext = path.extname(track.audioFile.filePath).slice(1); // Remove leading dot
  ...
  return {
    ...
    ext, // File extension (e.g., "flac", "mp3", "ogg")
  };
}
