...
export async function ingestMusic(
  ...
  // Initialize logger with config settings (debug flag overrides config)
  if (config.log) {
    logger.init({
      level: flags.debug ? "debug" : config.log.level,
...
async function preflightValidation(
  ...
  // Log LLM suggestion to file
  logger.info(`LLM analyzed setlist mismatch: ${audioFileCount} files vs ${setlistCount} songs`);
  if (suggestion.merges && suggestion.merges.length > 0) {
    logger.info(`LLM suggested ${suggestion.merges.length} merge(s): ${JSON.stringify(suggestion.merges)}`);
  }
  if (suggestion.splits && suggestion.splits.length > 0) {
    logger.info(`LLM suggested ${suggestion.splits.length} split(s): ${JSON.stringify(suggestion.splits)}`);
  }
  logger.info(`LLM confidence: ${(suggestion.confidence * 100).toFixed(0)}%`);
  ...
  if (needsUserInput) {
    ...
    if (!flags.batch && !flags["dry-run"]) {
      ...
      if (userInstructions && userInstructions.trim().length > 0) {
        logger.info(`User entered instructions: "${userInstructions.trim()}"`);
        ...
        if (parsedSuggestion.merges && parsedSuggestion.merges.length > 0) {
          logger.info(`Parsed ${parsedSuggestion.merges.length} merge(s) from user instructions: ${JSON.stringify(parsedSuggestion.merges)}`);
        }
        if (parsedSuggestion.splits && parsedSuggestion.splits.length > 0) {
          logger.info(`Parsed ${parsedSuggestion.splits.length} split(s) from user instructions: ${JSON.stringify(parsedSuggestion.splits)}`);
        }
        if (parsedSuggestion.modifiedSetlist) {
          logger.info(`Parsed modified setlist from user instructions: ${parsedSuggestion.modifiedSetlist.length} songs`);
        }
        ...
        if (parsedSuggestion.merges && parsedSuggestion.merges.length > 0) {
          onProgress(`Merges: ${JSON.stringify(parsedSuggestion.merges)}`);
        }
        if (parsedSuggestion.splits && parsedSuggestion.splits.length > 0) {
          onProgress(`Splits: ${JSON.stringify(parsedSuggestion.splits)}`);
        }
        if (parsedSuggestion.modifiedSetlist) {
          onProgress(`Modified setlist: ${parsedSuggestion.modifiedSetlist.length} songs`);
        }
        onProgress("=".repeat(60));
  ...
  if (suggestion.merges && suggestion.merges.length > 0) {
    ...
    if (!flags.batch && !flags["dry-run"]) {
      ...
      if (!confirmed) {
        // User declined - give them option to provide their own instructions
        onProgress("\nYou can provide alternative instructions instead.");
        onProgress("Examples:");
        onProgress("  - 'remove drum solo from setlist'");
        ...
        if (userInstructions && userInstructions.trim().length > 0) {
          ...
          if (parsedSuggestion.confidence >= 0.5) {
            ...
            onProgress("\nCould not parse your instructions with sufficient confidence.");
          }
        }

        // If we get here, user provided no valid input
        return {
          canProceed: false,
          manifest,
          reason: "User declined LLM suggestions and provided no valid alternative instructions.",
        };
      }
    }
...
async function processSingleArchive(
  ...
  try {
    ...
    onProgress("\nTagging files...");
    logger.info(`Tagging ${matched.length} file(s) with metadata`);
    printTagsSummary(matched, showInfo, bandConfig, onProgress);
    await tagAllTracks(matched, showInfo, bandConfig, onProgress);
    logger.info(`Successfully tagged ${matched.length} file(s)`);
...
async function copyToLibrary(
  ...
  const fileNameTemplate = getFileNameTemplate(matched, bandConfig);
  logger.info(`Copying ${matched.length} file(s) to library: ${targetDir}`);
  ...
  for (const m of matched) {
    ...
    logger.info(`Copied to library: ${fileName}`);
    copied++;
  }
  logger.info(`Finished copying to library: ${copied} copied, ${skipped} skipped`);
}
